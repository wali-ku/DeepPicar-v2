	...
	for access in ${accessType[@]}; do
		for num_of_corunners in `seq 1 ${corunners}`; do
			...
			if [ "${execution}" == "CPU" ]; then
				CUDA_VISIBLE_DEVICES='' taskset -c 0 python test-model.py 1 0 2>&1 | tee ${execution}_bw${num_of_corunners}_${access}.perf
			else
				taskset -c 0 python test-model.py 1 0 2>&1 | tee ${execution}_bw${num_of_corunners}_${access}.perf
			fi
			echo '-------------------------------------------------'
			killall bandwidth
		done
